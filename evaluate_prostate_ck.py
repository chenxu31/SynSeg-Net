...
def evaluate(args, device):
    ...
    test_data_s, test_data_t, test_label_s, test_label_t = common_prostate_ck.load_test_data(args.data_dir)
    ...
    netG_seg = networks.define_G(1, common_prostate_ck.NUM_CLASSES, 64, "resnet_9blocks", "instance", False,
    ...
    with torch.no_grad():
        ...
        fake_A_list = []
        fake_B_list = []
        dsc_list = np.zeros((test_data_t.shape[0], common_prostate_ck.NUM_CLASSES - 1), np.float32)
        assd_list = np.zeros((test_data_t.shape[0], common_prostate_ck.NUM_CLASSES - 1), np.float32)
        for i in range(test_data_t.shape[0]):
            pred = np.zeros([common_prostate_ck.NUM_CLASSES, test_data_t.shape[1], test_data_t.shape[2], test_data_t.shape[3]], np.float32)
            ...
            dsc = common_metrics.calc_multi_dice(pred, test_label_t[i], common_prostate_ck.NUM_CLASSES)
            assd = common_metrics.calc_multi_assd(pred, test_label_t[i], common_prostate_ck.NUM_CLASSES)

            dsc_list[i, :] = dsc
            assd_list[i, :] = assd

    msg = "dsc:%f/%f  assd:%f/%f" % (dsc_list.mean(), dsc_list.std(), assd_list.mean(), assd_list.std())
    for c in range(common_prostate_ck.NUM_CLASSES - 1):
    ...
    if args.output_dir:
        ...
        for i, im in enumerate(pred_list):
            common_prostate_ck.save_nii(im, os.path.join(args.output_dir, "pred_%d.nii.gz" % i))
            common_prostate_ck.save_nii(fake_A_list[i], os.path.join(args.output_dir, "syn_st_%d.nii.gz" % i))
            common_prostate_ck.save_nii(fake_B_list[i], os.path.join(args.output_dir, "syn_ts_%d.nii.gz" % i))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, default=r'F:\datasets\prostate_ck\h5_data')
