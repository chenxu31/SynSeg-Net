...
def evaluate(device, opt, net_seg, val_data_t, val_label_t, test_data_t, test_label_t):
    net_seg.eval()

    test_t_pred_list = []
    val_t_dsc_list = np.zeros((val_data_t.shape[0], common_prostate_ck.NUM_CLASSES - 1), np.float32)
    test_t_dsc_list = np.zeros((test_data_t.shape[0], common_prostate_ck.NUM_CLASSES - 1), np.float32)
    test_t_assd_list = np.zeros((test_data_t.shape[0], common_prostate_ck.NUM_CLASSES - 1), np.float32)
    with torch.no_grad():
        for i in range(val_data_t.shape[0]):
            t_pred = produce_result(device, val_data_t[i], net_seg, common_prostate_ck.NUM_CLASSES)
            t_pred = t_pred.argmax(0).astype(np.float32)
            t_dsc = common_metrics.calc_multi_dice(t_pred, val_label_t[i], num_cls=common_prostate_ck.NUM_CLASSES)
            val_t_dsc_list[i] = t_dsc

        for i in range(test_data_t.shape[0]):
            t_pred = produce_result(device, test_data_t[i], net_seg, common_prostate_ck.NUM_CLASSES)
            t_pred = t_pred.argmax(0).astype(np.float32)
            t_dsc = common_metrics.calc_multi_dice(t_pred, test_label_t[i], num_cls=common_prostate_ck.NUM_CLASSES)
            t_assd = common_metrics.calc_multi_assd(t_pred, test_label_t[i], num_cls=common_prostate_ck.NUM_CLASSES)
...
if __name__=='__main__':
    ...
    opt.crossentropy_weight = [1, 1, 1, 1]
    opt.task = "prostate"
    ...
    _, val_data_t, _, val_label_t = common_prostate_ck.load_val_data(opt.dataroot)
    test_data_s, test_data_t, test_label_s, test_label_t = common_prostate_ck.load_test_data(opt.dataroot)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataset):
            ...
            if total_steps % opt.print_freq == 0:
                ...
                train_dsc = common_metrics.calc_multi_dice(train_pred, data["Seg_one"].cpu().detach().numpy(),
                                                           common_prostate_ck.NUM_CLASSES)
                ...
                train_img = common_prostate_ck.generate_display_image(train_img, is_seg=False)
                train_seg = np.concatenate([data["Seg_one"].cpu().numpy(),
                                            np.expand_dims(train_pred, 1)], 3)
                train_seg = common_prostate_ck.generate_display_image(train_seg, is_seg=True)
                ...
                val_t_dsc, test_t_dsc, test_t_assd, test_t_pred_list = evaluate(device, opt, model.netG_seg, val_data_t, val_label_t, test_data_t, test_label_t)

                cur_dsc = val_t_dsc.mean()
                msg = "val_dsc:%f  test_dsc:%f/%f test_assd:%f/%f" % \
                      (cur_dsc, test_t_dsc.mean(), test_t_dsc.std(), test_t_assd.mean(), test_t_assd.std())
                print(msg)
                ...
                test_seg = np.expand_dims(np.concatenate([test_label_t[0], test_t_pred_list[0]], 2), 0)
                test_seg = common_prostate_ck.generate_display_image(test_seg, is_seg=True)
