...
class mmwhsSegDataset(BaseDataset):
    def initialize(self, opt):
        ...
        f_A = h5py.File(os.path.join(opt.dataroot, "mr_train.h5"), "r")
        f_B = h5py.File(os.path.join(opt.dataroot, "ct_train.h5"), "r")
    ...
    def __getitem__(self, index):
        ...
        if self.data_A is None:
            self.data_A = h5py.File(os.path.join(self.root, "mr_train.h5"), "r")["data"]
        if self.data_B is None:
            self.data_B = h5py.File(os.path.join(self.root, "ct_train.h5"), "r")["data"]
        if self.data_Seg is None:
            self.data_Seg = h5py.File(os.path.join(self.root, "mr_train.h5"), "r")["label"]
        ...
        A_img = numpy.array(self.data_A[A_id1, :, :, A_id2:A_id2 + 1], numpy.float32)
        B_img = numpy.array(self.data_B[B_id1, :, :, B_id2:B_id2 + 1], numpy.float32)
        Seg_img = numpy.array(self.data_Seg[A_id1, :, :, A_id2:A_id2 + 1], numpy.int64)
        ...
        Seg_imgs = torch.Tensor(self.opt.output_nc_seg, Seg_img.shape[1], Seg_img.shape[2])
        for i in range(self.opt.output_nc_seg):
