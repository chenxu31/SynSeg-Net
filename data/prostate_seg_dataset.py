...
class prostateSegDataset(BaseDataset):
    def initialize(self, opt):
        ...
        f_A = h5py.File(os.path.join(opt.dataroot, "train_plan.h5"), "r")
        f_B = h5py.File(os.path.join(opt.dataroot, "train_treat.h5"), "r")
    ...
    def __getitem__(self, index):
        ...
        if self.data_A is None:
            self.data_A = h5py.File(os.path.join(self.root, "train_plan.h5"), "r")["data"]
        if self.data_B is None:
            self.data_B = h5py.File(os.path.join(self.root, "train_treat.h5"), "r")["data"]
        if self.data_Seg is None:
            self.data_Seg = h5py.File(os.path.join(self.root, "train_plan.h5"), "r")["label"]
