...
def psnr(im1, im2, peakval=1.):
    ...
    snr = 10 * numpy.log10(peakval ** 2 / mse)
    return snr

def calc_multi_dice(result, gt, num_cls):
    assert num_cls > 2

    dice_list = [0.] * (num_cls - 1)
    ...
    for c in range(1, num_cls):
        ...
        dice_list[c - 1] = dc(_pred, _label)

    return dice_list

def calc_multi_assd(result, gt, num_cls):
    assert num_cls > 2

    assd_list = [0.] * (num_cls - 1)
    ...
    for c in range(1, num_cls):
        ...
        assd_list[c - 1] = assd(_pred, _label)
...
def assd(result, reference, voxelspacing=None, connectivity=1):
    ...
    if numpy.count_nonzero(reference) == 0:
        return 0
